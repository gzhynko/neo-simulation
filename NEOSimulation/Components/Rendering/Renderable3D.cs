...
namespace NEOSimulation.Components.Rendering
{
    public abstract class Renderable3D : RenderableComponent
    {
        ...
        {
        	get => new Vector3(LocalOffset, _positionZ);
        	set
        	{
        		_positionZ = value.Z;
        		SetLocalOffset(new Vector2(value.X, value.Y));
        	}
        }
        ...
        public Matrix WorldMatrix
        {
        	get
        	{
        		...
        		var pos = Position + new Vector3(_localOffset.X, -_localOffset.Y, 0f);
        		var worldMatrix = Matrix.CreateScale(Scale) * Matrix.CreateTranslation(pos.X, -pos.Y, pos.Z) * rotationMatrix;
        		return worldMatrix;
        	}
        }
    }
